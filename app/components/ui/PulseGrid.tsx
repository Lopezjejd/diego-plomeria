...
/* ========= Fragment shader BLUE (fixed: init z/d/f; safe divides) ========= */
const SHADER_SRC_BLUE = `#version 300 es
...
// mesh-gradient: 4 deep blue points
...
/* ========= Fragment shader RED variant ========= */
const SHADER_SRC_RED = `#version 300 es
precision highp float;
...
// mesh-gradient: 4 deep red points
vec3 meshGradient(vec2 uv){
  vec2 p0=vec2(-0.70,-0.45), p1=vec2(0.75,-0.35), p2=vec2(-0.65,0.65), p3=vec2(0.80,0.55);
  vec3 c0=vec3(0.26,0.05,0.05);  // deep crimson
  vec3 c1=vec3(0.36,0.08,0.08);  // dark red
  vec3 c2=vec3(0.22,0.03,0.03);  // abyss red
  vec3 c3=vec3(0.40,0.10,0.10);  // ruby
  float e=2.0;
  float w0=pow(1.0/(0.2+distance(uv,p0)),e);
  float w1=pow(1.0/(0.2+distance(uv,p1)),e);
  float w2=pow(1.0/(0.2+distance(uv,p2)),e);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  vec3 baseDeep = vec3(0.12,0.03,0.03);
  vec3 baseTint = vec3(0.18,0.05,0.05);
  float vgrad   = smoothstep(-0.92, 0.55, -uv.y);
  vec3  bg      = mix(baseDeep, baseTint, vgrad);
  bg            = mix(bg, meshGradient(uv), MESH_AMT);
  float rad     = length(uv);
  ...
  vec3 lineThin  = vec3(0.95,0.58,0.58);
  vec3 lineMajor = vec3(1.00,0.78,0.78);
  ...
  if (ASCII_AMT > 0.001) {
    ...
    float nearMajor = major;                   
    float asciiMask = asciiLineSel * nearMajor;
    vec3 asciiColor = mix(vec3(1.0,0.50,0.50), meshGradient(uv), 0.25);
...
/* ========= Default export: modified for wrapper use with variant support ========= */
interface PulseGridProps {
    children?: React.ReactNode;
    variant?: "blue" | "red";
}

export default function PulseGridBackground({ children, variant = "blue" }: PulseGridProps) {
    const shaderSource = variant === "red" ? SHADER_SRC_RED : SHADER_SRC_BLUE;
    ...
    return (
        ...
        >
            <ShaderCanvas fragSource={shaderSource} />
